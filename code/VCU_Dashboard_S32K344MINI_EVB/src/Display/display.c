...
void newDisplayUpdate(uint8_t Acceleration, uint8_t Brake, uint8_t Battery_Percentage, uint16_t Motor_Temperature, uint16_t Inverter_Temperature, uint8_t Speed, uint16_t Cell_Voltage, uint16_t Cell_Temperature, uint16_t Total_Current, uint16_t Total_Voltage, uint8_t Minutes, uint8_t Seconds, uint32_t Miliseconds, uint8_t Witnesses){
	...
	if(rd8(REG_DLSWAP) == 0){
		...
		FTDI_Array[arrayIndex++] = vertex2f(UPPER_BORDER_X + UPPER_BORDER_WIDTH - 100, UPPER_BORDER_Y);
		FTDI_Array[arrayIndex++] = vertex2f(UPPER_BORDER_X + UPPER_BORDER_WIDTH + UPPER_BORDER_OFFSET - 100, 0);
		...
		if(Motor_Temperature >=10U){
			FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X - 2, MOTOR_TEMP_POSITION_Y + 2, LARGE_FONT, (Motor_Temperature / 10U) + '0');
		}
		FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X + 20, MOTOR_TEMP_POSITION_Y + 2, LARGE_FONT, (Motor_Temperature % 10) + '0');
		FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X + 45, MOTOR_TEMP_POSITION_Y + 2, CELSIUS_SYMBOL, 'o');
		FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X + 52, MOTOR_TEMP_POSITION_Y + 2, LARGE_FONT, 'C');

		FTDI_Array[arrayIndex++] = color_rgb(255, 255, 255);				//50, 255, 150
		//Dynamic Text for Motor Temperature
		if(Motor_Temperature >=10U){
			FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X, MOTOR_TEMP_POSITION_Y, LARGE_FONT, (Motor_Temperature / 10U) + '0');
		}
		FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X + 22, MOTOR_TEMP_POSITION_Y, LARGE_FONT, (Motor_Temperature % 10) + '0');
		FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X + 47, MOTOR_TEMP_POSITION_Y, CELSIUS_SYMBOL, 'o');
		FTDI_Array[arrayIndex++] = vertex2ii(MOTOR_TEMP_POSITION_X + 54, MOTOR_TEMP_POSITION_Y, LARGE_FONT, 'C');
		FTDI_Array[arrayIndex++] = vertex_translate_x(0);
