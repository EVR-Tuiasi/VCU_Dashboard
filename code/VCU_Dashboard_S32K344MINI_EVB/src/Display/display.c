 ...
 *                                      LOCAL VARIABLES
==================================================================================================*/
static DisplaySystemState_t display_system_state = INITIALIZING;
static bool display_error_flag = false;
static uint8_t contorInitializare = 0;
static uint16_t contorUpdate = 0;
volatile uint16_t vectorIndiciUpdate[2048] = {0};
...
static void Display_State_Update(void)
{
	switch (display_system_state)
	{
	case INITIALIZING:
		/* Daca nu avem erori si registrul ID raspunde corect, atunci totul e functional */
		if (display_system_state == false && contorInitializare == 26)
		{
			display_system_state = OPERATIONAL;
		}
		else if (display_error_flag == true)
		{
			display_system_state = DISPLAY_ERROR;
		}
		break;
	case DISPLAY_ERROR:
		/* Starea se va schimba in INITIALIZING la finalul Display_Recover */
		if (display_error_flag == false)
		{
			display_system_state = INITIALIZING;
		}
		break;
	case OPERATIONAL:
		/* Daca nu se ridica flagul de eroare sau daca displayul nu mai raspunde */
		if (display_error_flag == true || rd8(REG_ID) != 0x7C)
		{
			display_system_state = DISPLAY_ERROR;
		}
		break;
...
static void Display_Recover(void)
{
	...
	if(rd(REG_ID) == 0x7C && contorInitializare == 26)
	{
...
void Display_Init(void){
	if (vectorIndiciUpdate[1] == 0)
	{
		for(uint16_t i = 0; i < 2048; i++)
		{
			vectorIndiciUpdate[i] = i;
		}
	}

	switch (contorInitializare)
	{
	case 0:
...
void Display_Update(uint8_t Acceleration, uint8_t Brake, uint8_t Battery_Percentage, uint16_t Motor_Temperature, uint16_t Inverter_Temperature, uint8_t Speed, uint16_t Cell_Voltage, uint16_t Cell_Temperature, uint16_t Total_Current, uint16_t Total_Voltage, uint8_t Minutes, uint8_t Seconds, uint32_t Miliseconds){
	...
	{
	case INITIALIZING:
		/* Executa secvential pasii de initializare la fiecare apel de task */
		/* La fiecare apel de Display_State_Update va stii daca trece in operational sau nu */
		Display_Init();
		break;
	...
	case OPERATIONAL:
		...
		case vectorIndiciUpdate[contorUpdate]:
			if((rd8(REG_DLSWAP) == 0) && (Total_Current >= 300U)){
		...
		case vectorIndiciUpdate[contorUpdate]:
			if((rd8(REG_DLSWAP) == 0) && (Total_Current >= 600U)){
				wr32(RAM_DL + (index+=4), color_rgb(255, 0, 0));
			}
			contorUpdate++;
			break;
	...
	/* Evaluam tranzitiile de stare la finalul fiecarui apel */
